...
export default function PricingClient({ lang }: { lang: string }) {
  ...
  const L = lang as Parameters<typeof t>[0];
  ...
  async function choose(id: 'free' | 'pro') {
    ...
    setQrImgOk(true);
    try {
      const res = await subscribe(id, id === 'pro' ? method : undefined, L);
      if (res.url) {
        // Stripe / 支付宝：前端跳转支付网关
        setMsg(t(L, 'payment.redirect'));
      ...
      setMsg(e.message || t(L, 'common.error'));
    } finally {
      setBusy(false);
    }
  }
      ...
      <div className="grid md:grid-cols-2 gap-6">
        {PLANS.map((p) => (
          <div key={p.id} className="border rounded-2xl p-6 bg-white flex flex-col">
            <h2 className="text-xl font-bold">{p.id === 'pro' ? t(L, 'pricing.pro') : t(L, 'pricing.free')}</h2>
            ...
            {p.id === 'pro' && (
              <div className="mt-4">
                <div className="text-xs text-slate-400 mb-1">{t(L, 'payment.method')}</div>
