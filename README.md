...
- **SEO+GEO 优化（Stage 35）**：在 Stage 34 的 SSG/ISR 基础上补齐**生成式引擎优化（GEO）**能力——① 新增 `app/llms.txt/route.ts` 与 `app/llms-full.txt/route.ts`（llmstxt.org 规范），为 ChatGPT/Perplexity/Claude 等 AI 爬虫提供站点结构化导览与全量人物 URL 索引（13 语），`force-static`/`revalidate=300`；② 人物详情页新增 **FAQPage JSON-LD**（由事实自动生成：Who / When born / Known for / Nationality / Net worth），答案引擎高频引用；③ `sitemap.ts` 全量 URL 补 **hreflang x-default 交替链接**（产物核验 871 处），人物/首页 `generateMetadata` 同步加 `x-default`，国际版语言信号完整；④ `robots.ts` 显式放行主流 AI 爬虫（GPTBot / ClaudeBot / Google-Extended / Applebot-Extended / PerplexityBot 等）并指向 sitemap；⑤ 新增 `/feed.xml` **RSS 2.0** 新鲜度信号，经首页 `alternates.types` 暴露；⑥ 首页补 **Organization** 发布者实体、`WebSite`/`Person` 加 `isAccessibleForFree:true`，强化 E-E-A-T 权威信号。实测：`typecheck` 绿；`next build` 成功（793 SSG 页 + 3 个 GEO 路由处理器均编译）；产物核验——人物页含 FAQPage + 14 条 hreflang `<link>`、首页含 Organization+SearchAction、sitemap.xml 871 处 x-default、llms.txt/llms-full.txt/feed.xml 正文正确，`[ui]` 告警 0，零新增依赖、零新 UI 键。
